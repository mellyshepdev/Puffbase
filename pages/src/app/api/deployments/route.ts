...
export async function GET(request: NextRequest) {
  ...
  if (!hasScope(ctx.scopes, "deployments:read")) return NextResponse.json({ error: "pufftoken lacks the deployments:read scope" }, { status: 403 });
...
export async function POST(request: NextRequest) {
  const ctx = await requestAccount(request);
  if (!ctx) return NextResponse.json({ error: "Not signed in" }, { status: 401 });
  if (!hasScope(ctx.scopes, "deployments:write")) return NextResponse.json({ error: "pufftoken lacks the deployments:write scope" }, { status: 403 });
  try {
